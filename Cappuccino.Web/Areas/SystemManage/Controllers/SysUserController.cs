        ...
        public JsonResult GetList(SysUserModel viewModel, PageInfo pageInfo)
        {
            var coreSql = $@"
                SELECT 
                    ...
                    ISNULL(roles.RoleNames, '') AS RoleName
                FROM dbo.SysUser u
                LEFT JOIN dbo.SysDepartment d ON u.DepartmentId = d.Id
                LEFT JOIN (
                    -- 先按 UserId 分组，用 FOR XML PATH 拼接角色名
                    SELECT 
                        ur.UserId,
                        STUFF((
                            SELECT ',' + r.Name
                            FROM dbo.SysUserRole ur2
                            INNER JOIN dbo.SysRole r ON ur2.RoleId = r.Id
